...
formal proof, lab report, incident postmortem, courtroom exchange, customer-support ticket, recipe, assembly manual, dialogue between two specialists, child's riddle, exam question, data table with a prompt, API documentation, field notebook, diary entry, terse news brief, game rulebook, engineering spec sheet, patient chart, negotiation memo, step-by-step tutorial, grant proposal, safety data sheet, arbitration ruling, product review, onboarding checklist, release notes.
...
add a red herring that looks decisive but is not; introduce a constraint that eliminates the intuitive answer; make the shortest-looking path wrong; require noticing something absent rather than present; embed a second, distractor question; make two plausible answers hinge on one detail; invert the usual direction of the relationship; arrange sufficient data so it looks incomplete; hide the decisive fact inside a plausible distractor; make a correct-looking shortcut yield a subtly wrong value; require rejecting a premise that is stated as settled fact.
...
- **Registers:** at least 16 of the register deck represented.
- **Domains:** at least 60 distinct domains represented (the deck now holds ~115).
